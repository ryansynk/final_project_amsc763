...
int qr(Matrix *A, Matrix *Q, Matrix *R) {
    ...
    //optional?
    //copy_matrix(R, R_submatrix);
    //copy_matrix(Q, Q_submatrix);
    ...
    for (int k = 0; k < A->cols; k++) {
        ...
        gemm(MATRIX_OP_N, MATRIX_OP_N, Q_submatrix, &v, Q_submatrix, -beta, 0.0);       // Q[:, k:m] <-- -beta (Q[:, k:m] @ v)      
    ...
    R_submatrix->rows = R->rows;
    R_submatrix->cols = R->rows;
